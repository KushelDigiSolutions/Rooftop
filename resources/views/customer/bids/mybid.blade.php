...
<script>
    ...
    $.ajax({
        url: '/my-bid-data', // API endpoint to fetch the data
        method: 'GET',
        data: {
                    ...
                    row += `<td>
                                <div>
                                    ...
                                    <ul class="dropdown-menu">
									   ...
									   ${(response.role === 'Admin' || response.role === 'Super Admin' || response.role === 'Franchise') && appnt.status == 0 ? `
                                          <li><a href="#" class="small dropdown-item" data-id="${appnt.id}" onclick="editQuotation(this)">Edit </a></li>
                                          
                                      ` : ''}
                                        <li><a href="bids/download_quotes/${appnt.id}" class="dropdown-item small download_quotation_btn" data-quotation-id="${appnt.id}">View</a></li>
                                    </ul>
                                </div>
                            </td>`;
                    row += `</tr>`;
	...
	function editQuotation(element) {
            const id = element.getAttribute('data-id');
            window.location.href = "{{ url('bids/edit') }}/" + id;
        } 
</script>
@endsection