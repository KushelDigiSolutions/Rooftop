    ...
    <div class="dataOverview mt-3">
        <div>
            <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="pills-shipped-tab" data-bs-toggle="pill" data-bs-target="#pills-shipped" type="button" role="tab" aria-controls="pills-shipped" aria-selected="false">Installation Pending <span class="fw-normal small">({{$installationCount}})</span></button>
                </li>
                @else
                <li class="nav-item" role="presentation">
                    <button class="nav-link active" id="pills-pending-tab" data-bs-toggle="pill" data-bs-target="#pills-pending" type="button" role="tab" aria-controls="pills-pending" aria-selected="false">Installation Pending <span class="fw-normal small">({{$pendingCount}})</span></button>
                </li>
                @if(in_array(Auth::user()->getRoleNames()[0], ['Admin', 'Super Admin','Help Desk']))
                <li class="nav-item" role="presentation">
        ...
        <div class="tab-content" id="pills-tabContent">
            ...
            <div class="tab-pane fade" id="pills-shipped" role="tabpanel" aria-labelledby="pills-schedule-tab" tabindex="0">
                <!-- This content will be dynamically populated -->
            </div>
        ...
        <div class="table-responsive">
            <table class="table projectsTable" id="ordersTable">
                <thead>
                    <tr>
                        <th>S/N</th>
                        ...
                        <th class="installation-date-header">Shipped Date & Time</th>
                        <th>Action</th>
                    </tr>
                </thead>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form action="{{url('orders/update_schedule')}}" method="POST">
                @csrf
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="approveFranchiseModalLabel">Assign To Fulfillment Desk</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        @endphp
                        <input type="datetime-local" name="dateFilter" id="dateFilter" placeholder="Filter by date" value="{{ request('dateFilter') }}" min="{{ $now }}" class="form-control me-3 w-100" oninput="validateDate(this)">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="secondary-btn" data-bs-dismiss="modal">Cancel</button>
...
<div class="modal fade" id="statusOrderUpdate" tabindex="-1" aria-labelledby="statusUpdateOrderLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form action="{{route('shipped.status')}}" method="POST">
                @csrf
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="status" class="form-label">Track Details<span class="requried">*</span></label>
                        <input type="text" class="form-control w-100" name="track_detail" requried>
...
<script>
    ...
    $('#ordersTable tbody').html('<tr><td colspan="8" class="text-center">Loading...</td></tr>');
    $.ajax({
        ...
        success: function(response) {
            ...
            // Always show the installation date header
            ...
            if (response.data && response.data.length > 0) {
                ...
                $.each(response.data, function(idx, order) {
                    var row = '<tr>';
                    row += '<td>' + (idx + 1) + '</td>';
                    row += '<td>' + order.appointment.uniqueid + '</td>';
                    row += '<td>' + order.quotation_data.unique_id + '</td>';
                    ...
                    // Always include installation date column (show N/A if not available)
                    ...
                    if (response.role == 'Fulfillment Desk') {
                        //check_status = '1';
                        check_status = (check_status === '0') ? '1' : check_status;
                    }
                    ...
                    switch (check_status) {
                        case '0':
                            ...
                            if (response.role == 'Help Desk' || response.role == 'Super Admin') {
                                actions += '<li><a href="javascript:" class="dropdown-item small update-orders-btn" data-orders-id="' + order.id + '" onclick="confirmAssign(\'' + order.id + '\')">Assign Fulfillment Desk</a></li>';
                            }
                            break;
                        ...
                        case '1':
                            ...
                            actions += '<li><a href="orders/download_order/' + order.quotation_id + '" class="dropdown-item small download_invoice_btn" data-quotation-id="' + order.quotation_id + '">Download Invoice</a></li>';
                            actions += '<li><a href="quotations/download_quotes/' + order.quotation_id + '" class="dropdown-item small download_quotation_btn" data-quotation-id="' + order.quotation_id + '">Download Quotation</a></li>';
                            if (response.role == 'Fulfillment Desk' || response.role == 'Super Admin') {
                                actions += '<li><a href="javascript:" class="dropdown-item small update-orders-btn" data-orders-id="' + order.id + '" onclick="statusUpdate(\'' + order.id + '\')">Order Shipped</a></li>';
                            }
                            break;
                        ...
                        case '3':
                            ...
                            actions = '<li><a href="javascript:" class="dropdown-item small update-orders-btn" data-orders-id="' + order.id + '" onclick="statusUpdate(\'' + order.id + '\')">Update Status</a></li>';
                            }
                            break;    
            ...
            // Initialize DataTables regardless of data
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        $('#FranciseView .offcanvas-body table tbody').html(`
                                ...
                                <tr><th>Appointment Id</th><td>${orders.appointment.uniqueid || 'N/A'}</td></tr>
                                ...
                                <tr><th>Quotation Id</th><td>${orders.quotation_data.unique_id || 'N/A'}</td></tr>
...
function validateDate(input) {
    ...
    if (inputDate < minDate) {
        alert('Please select a date and time after {{ $now }}');
        input.value = '';
    }
}
</script>
