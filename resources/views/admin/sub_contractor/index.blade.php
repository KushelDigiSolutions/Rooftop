...
<div class="dataOverviewSection mt-3">
    <div class="section-title">
        <h6 class="fw-bold m-0">All Sub Contractor <span class="fw-normal text-muted"></span></h6>
        <a href="{{ route('sub_contractor.create') }}" class="primary-btn addBtn">+ Add Sub Contractor</a>
    </div>
        ...
        <div>
            ...
            <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
				...
				<li class="nav-item" role="presentation">
                    <button class="nav-link active" id="pills-all-tab" data-bs-toggle="pill" data-bs-target="#pills-all" type="button" role="tab" aria-controls="pills-all" aria-selected="true">All <span class="fw-normal small"></span></button>
                </li>
				
				<li class="nav-item" role="presentation">
                    <button class="nav-link " id="pills-pending-tab" data-bs-toggle="pill" data-bs-target="#pills-pending" type="button" role="tab" aria-controls="pills-pending" aria-selected="true">Pending <span class="fw-normal small"></span></button>
                </li>
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="pills-active-tab" data-bs-toggle="pill" data-bs-target="#pills-active" type="button" role="tab" aria-controls="pills-active" aria-selected="true">Active <span class="fw-normal small"></span></button>
                </li>
...
<script>
    ...
    $.ajax({
        url: "{{route('getSubcontractorData')}}", // API endpoint to fetch the data
        method: 'GET',
        data: {
        ...
        success: function(response) {
            ...
            if (response.data && response.data.length > 0) {
                $.each(response.data, function(idx, appnt) {
                    var row = '<tr>';
                    row += '<td>' + (idx + 1) + '</td>';
                    row += '<td>' + appnt.contact_person + '</td>';
                    row += '<td>' + appnt.company_name + '</td>';
                    row += '<td>' + appnt.phone + '</td>';
                    ...
                    switch (appnt.status) {
                        ...
                        case '1':
                            viewType = 'pending';
                            statusBadge = '<span class="badge badge-pending text-danger">Pending</span>';
                            actions += '<li><a href="/sub-contractor/edit/' + appnt.id + '" class="dropdown-item">Edit</a></li>';
							actions += '<li><a href="javascript:void(0);" onclick="activateSubcontractor(' + appnt.id + ')" class="dropdown-item text-success">Activate</a></li>';
                            actions += '<li><a href="javascript:void(0);" onclick="showDeleteModal(' + appnt.id + ')" class="dropdown-item text-danger">Delete</a></li>';
                            break;
...
<script>
    function deleteSubcontractor(id) {
        if (confirm("Are you sure you want to delete this subcontractor?")) {
            $.ajax({
                url: '/subcontractors/' + id + '/delete',
                type: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
                success: function (response) {
                    toastr.success('Sub Contractor deleted successfully!');
                    // Refresh or remove row from table (optional)
                    // Example: $('#row-' + id).remove();
                    location.reload();
                },
                error: function () {
                    toastr.error('Failed to delete subcontractor.');
                }
            });
        }
    }
...
<script>
    ...
    $('#confirmDeleteBtn').click(function () {
        if (deleteId) {
            $.ajax({
                url: '/subcontractors/' + deleteId + '/delete',
                type: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
	...
	function activateSubcontractor(id) {
  
    // Show loader
    $('#ajax-loader').show();

    $.ajax({
        url: '/sub-contractor/active/' + id,
        method: "POST",
        headers: {
	...
	function activateSubcontractor2(id) {
    if (!confirm("Are you sure you want to activate this subcontractor?")) return;
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            location.reload(); // or refetch your table
        },
        error: function (xhr) {
