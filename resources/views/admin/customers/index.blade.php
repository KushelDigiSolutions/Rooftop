    ...
    <div class="dataOverviewSection mt-3">
        ...
        <a href="{{ route('customer.create') }}" class="primary-btn addBtn">+ Add Customer</a>
        </div>
        ...
        <div class="dataOverview mt-3">
            <div>
                ...
                <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
					...
					<li class="nav-item" role="presentation">
                        <button class="nav-link" id="pills-approve_contract-tab" data-bs-toggle="pill" data-bs-target="#pills-approve_contract" type="button" role="tab" aria-controls="pills-approve_contract" aria-selected="false">Signed Contract<span class="fw-normal small"></span></button>
                    </li>
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="pills-active-tab" data-bs-toggle="pill" data-bs-target="#pills-active" type="button" role="tab" aria-controls="pills-active" aria-selected="false">Active <span class="fw-normal small"></span></button>
                    </li>
    ...
    <div class="modal fade" id="rejectModal" tabindex="-1" aria-labelledby="rejectModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="rejectModalLabel">Move To In Progress</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    Are you sure you ?
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-dark" id="confirmRejectBtn">
                        ...
                        <span id="rejectBtnText"  >Submit</span>
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form action="{{ route('appointments.reassign') }}" method="POST" id="re_Assign">
                    ...
                    <div class="modal-body">
                        <input type="hidden" class="appointmentId1" name="job_id">
                        <div class="mb-3">
                            <!-- <label for="franchise" class="form-label">Select Franchise<span class="requried">*</span></label> -->
...
<div class="modal fade" id="convertModel" tabindex="-1" aria-labelledby="approveFranchiseModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form action="{{ route('closeAndProspect') }}" method="POST">
                    @csrf
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="approveFranchiseModalLabel">Close & Prospect</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="date" class="form-label">Customer Name<span class="requried">*</span></label>

                            <input type="text" name="name" id="nameField" placeholder="Customer Name" value="{{ request('dateFilter') }}" min="{{ $now }}" class="form-control me-3 w-100 nameField">
                            <div class="error" style="color: red;"></div>
                        </div>

                        <div class="mb-3">
                            <label for="date" class="form-label">Email<span class="requried">*</span></label>
                            <input type="text" name="email" id="emailField" placeholder="Email" value="{{ request('dateFilter') }}" min="{{ $now }}" class="form-control me-3 w-100 emailField">
                            <div class="error" style="color: red;"></div>
                        </div>

                        <div class="mb-3">
                            <label for="date" class="form-label">Phone Number<span class="requried">*</span></label>
                            <input type="text" name="phone" id="mobileField" placeholder="Phone Number" value="{{ request('dateFilter') }}" min="{{ $now }}" class="form-control me-3 w-100 mobileField">
                            <div class="error" style="color: red;"></div>
                        </div>

                        <div class="mb-3">
                            <label for="date" class="form-label">Status<span class="requried">*</span></label>
                            <select name="status" id="" class="form-control me-3 w-100">
                                <!-- <option value="7">In Progress</option> -->
                                <option value="8">Closed & Prospect</option>
                                <!-- <option value="10">Closed & Dead</option> -->
                            </select>
                            <div class="error" style="color: red;"></div>
                        </div>
...
<div class="modal fade" id="closeAndDeadModel" tabindex="-1" aria-labelledby="approveFranchiseModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <form action="{{ route('closeAndDead') }}" method="POST">
                    @csrf
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="approveFranchiseModalLabel">Close & Dead</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
    ...
    <script>
		...
		function close_prospect(appointmentId) {
        $('#convertModel').modal('show');
        $('.appointmentId').val(appointmentId);
        // alert(appointmentId);
        let selectId = "#approve_franchise";
        get_franchise_list(appointmentId,selectId);
		...
		function close_dead(appointmentId) {
        $('#closeAndDeadModel').modal('show');
        $('.appointmentId').val(appointmentId);
        // alert(appointmentId);
        let selectId = "#approve_franchise";
        get_franchise_list(appointmentId,selectId);
                        ...
                        switch (appnt.status) {
                            ...
                            case '8':
                                ...
                                // actions += '<li><a href="javascript:" class="dropdown-item small approve-appointment-btn" data-appointment-id="' + appnt.id + '" onclick="confirmAssign(\'' + appnt.id + '\')">Assign Lead</a></li>';
                                 actions += '<li><a href="javascript:" class="dropdown-item small approve-appointment-btn" data-appointment-id="' + appnt.id + '" onclick="reject(\'' + appnt.id + '\')">Sent To In Progress</a></li>';
                                break;
                            ...
                            case '3':
                                ...
                                if (appnt.quotation && appnt.quotation.length > 0) {
                                actions += '<li><a href="#" class="dropdown-item small approve-appointment-btn" data-qoutation_id="' + appnt.quotation[0].id + '" data-id="' + appnt.id + '" onclick="createContract(this)">Create Contract</a></li>';
                                }
                                break;
        ...
        function createContract(element) {
            const id = element.getAttribute('data-id');
            const qoutation_id = element.getAttribute('data-qoutation_id');
            window.location.href = "{{ url('/contract/create') }}/" + id+'/'+qoutation_id;
        }
        ...
        $(document).ready(function() {
            $(document).on('click', '[id^="open-appointment-details-"]', function() {
                var customer_id = $(this).data('id'); // Get franchise ID dynamically
				//alert(customer_id);
                var franchiseType = $(this).data('checktype'); // Get franchise ID dynamically
                $.ajax({
                    url: '/appointment/details/' + customer_id + '/' + franchiseType, // Make sure the URL is correct
                    method: 'GET',
                    success: function(response) {
