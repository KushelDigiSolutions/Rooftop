            ...
            <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link " id="pills-inactive-tab" data-bs-toggle="pill"
                        data-bs-target="#pills-inactive" type="button" role="tab" aria-controls="pills-inactive"
                        aria-selected="false">Deactiave <span
                            class="fw-normal small">({{ count($deactiveFranchises) }})</span></button>
                </li>
            </ul>
        </div>
        <div class="tab-content" id="pills-tabContent">
            <!-- for inaction -->
            <div class="tab-pane fade " id="pills-inactive" role="tabpanel" aria-labelledby="pills-confirm-tab"
                tabindex="0">
                <div class="table-responsive">
                    <table class="table" id="deactiveranchise">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            @foreach($deactiveFranchises as $idx => $franchise)
                                <tr>
                                    <td>{{ $idx + 1 }}</td>
                                    ...
                                    <td>
                                        @if ($franchise->user->status === 'active')
                                    ...
                                    <td>
                                        <div>
                                            ...
                                            <ul class="dropdown-menu">
                                                ...
                                                <li>
                                                    <a href="javascript:" class="dropdown-item change-status-btn" data-id="{{ $franchise->user->id }}" data-status="{{ $franchise->user->status }}">
                                                        {{ $franchise->user->status === 'active' ? 'Mark Inactive' : 'Mark Active' }}
                                                    </a>
                                                </li>
                                            </ul>
                                        </div>
            ...
            <div class="tab-pane fade" id="pills-rejected" role="tabpanel" aria-labelledby="pills-rejected-tab"
                tabindex="0">
                <div class="table-responsive">
                    <table class="table" id="franchiserejectTable">
                        <thead>
                            <tr>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form id="rejectFranchiseForm" method="POST"
                ...
                <div class="modal-body">
                    <p>Are you sure you want to reject this franchise?</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="secondary-btn" data-bs-dismiss="modal">Cancel</button>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#franchiserejectTable').DataTable({
            responsive: true,
            language: {
...
<script>
    $(document).ready(function () {
        $('.change-status-btn').on('click', function () {
            ...
            $.ajax({
                url: "{{ route('franchise.user.status.update') }}",
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    user_id: userId,
