                ...
                <form id="formAuthentication" method="POST" action="{{ route('custom.register.submit') }}" autocomplete="off">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-1 w-100 position-relative">
                            ...
                            <!--<input type="password" id="password" name="password" class="password-group form-control w-100" required autocomplete="paswword_new">-->
                            <!--<button class="btn bg-transparent password-group-btn border-0 toggle-password" type="button" data-target="password">-->
                            <!--        <i class="bi bi-eye-slash"></i>-->
                            <!--    </button>-->
                            <div class="d-flex w-100 justify-content-between align-items-start">
                                    <div class="w-100">
                                        <input id="password" type="password" class="form-control password-group @error('password') is-invalid @enderror" name="password" required autocomplete="new-password">
                                    </div>
                                    <button class="btn password-group-btn bg-transparent border-0 toggle-password" type="button" data-target="password">
                                    <i class="bi bi-eye-slash"></i>
                                    </button>
                                </div>
                        ...
                        <div class="mb-1 w-100 position-relative">
                            ...
                            <div class="d-flex w-100 justify-content-between align-items-start">
                                <div class="w-100">
                                    <input type="password" id="password_confirmation" name="password_confirmation" class="form-control password-group" required>
                                </div>
                                <button class="btn bg-transparent password-group-btn border-0 toggle-password" type="button" data-target="password_confirmation">
                                    <i class="bi bi-eye-slash"></i>
                                </button>
                            </div>
...
<script>
    $(document).ready(function () {
        ...
        // jQuery Validation for User Forms
            ...
            /// Handle modal opening and set the correct form action
        $('#changePasswordModal').on('show.bs.modal', function (event) {
            var button = $(event.relatedTarget); // Button that triggered the modal
            var userId = button.data('id'); // Extract the user ID from the data-* attributes
        ...
        $("#changePasswordForm").validate({
            ...
            errorPlacement: function (error, element) {
                ...
                var customErrorDiv = $("#" + fieldName + "_error");

                if (customErrorDiv.length) {
                    customErrorDiv.html(error); // Set the error inside the existing div
                } else {
                    error.addClass("form-text text-danger xsmall");
                    error.insertAfter(element); // fallback if no custom div found
                }
            },
        ...
        // Handle form submission via AJAX to prevent modal close on errors
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            showSuccess(response.message); // Optional: success message
                        }
                    },
                    error: function (xhr) {
