...
<div class="dataOverviewSection mt-3">
    <div class="section-title">
        <h6 class="fw-bold m-0">All Leads <span class="fw-normal text-muted"></span></h6>
        <a href="{{ route('leads.create') }}" class="primary-btn addBtn">+ Add Leads</a>
    </div>
...
<div class="modal fade" id="convertModel" tabindex="-1" aria-labelledby="approveFranchiseModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form action="{{ route('convertToCustomer') }}" method="POST">
                @csrf
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="approveFranchiseModalLabel">Convert To Customer</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" id="nameField" placeholder="Filter by date" value="{{ request('dateFilter') }}" min="{{ $now }}" class="form-control me-3 w-100">
                        <div class="error" style="color: red;"></div>
                    </div>

                    <div class="mb-3">
                        <label for="date" class="form-label">Email<span class="requried">*</span></label>
                        <input type="text" name="email" id="emailField" placeholder="Filter by date" value="{{ request('dateFilter') }}" min="{{ $now }}" class="form-control me-3 w-100">
                        <div class="error" style="color: red;"></div>
                    </div>

                    <div class="mb-3">
                        <label for="date" class="form-label">Phone Number<span class="requried">*</span></label>
                        <input type="text" name="phone" id="mobileField" placeholder="Filter by date" value="{{ request('dateFilter') }}" min="{{ $now }}" class="form-control me-3 w-100">
                        <div class="error" style="color: red;"></div>
                    </div>
...
<script>
    ...
    function convert(appointmentId) {
                ...
                $.each(response.data, function(idx, appnt) {
                    ...
                    switch (appnt.status) {
                        case '1':
                            ...
                            actions += '<li><a href="/lead/edit/' + appnt.id + '" class="dropdown-item">Edit</a></li>';
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        $('#FranciseView .offcanvas-body table tbody').html(`
                                ...
                                <tr><th>Type of Property</th><td>${franchise.type_property || 'N/A'}</td></tr>
                                <tr><th>Requirement Type</th><td>${franchise.requirement_type || 'N/A'}</td></tr>
								...
								<tr><th>Referral By</th><td>${franchise.users && franchise.users.name ? franchise.users.name : 'N/A'}</td></tr>
                            `);
