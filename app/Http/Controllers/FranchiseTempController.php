...
use App\Mail\FranchiseDeactivatedMail;
use App\Mail\FranchiseActivatedMail;
...
class FranchiseTempController extends Controller
{
    ...
    public function index()
    {
        ...
        ->orderBy('id', 'desc')
        ->get();
        $deactiveFranchises = Franchise::with('user')
        ->whereHas('user', function ($query) {
        ...
        return view("admin.franchise.approval", [
            ...
            "deactiveFranchises" => $deactiveFranchises,
    ...
    public function approve($id)
    {
        ...
        $this->whatsAppService->sendMessage('91'.$franchiseTemp->mobile, 'franchise_confirmed', $parameters);
        // end send whatsaap Message
    ...
    public function updateUserStatus(Request $request)
    {
        ...
        $user->save();
         ...
         // Send email if user is inactivated
           

            if (!empty($user->email)) {
                if ($request->status === 'inactive') {
                    Mail::to($user->email)->send(new FranchiseDeactivatedMail($user));
                } elseif ($request->status === 'active') {
                    Mail::to($user->email)->send(new FranchiseActivatedMail($user));
                }
            }
