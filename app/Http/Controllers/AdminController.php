...
class AdminController extends Controller
{
    public function dashboard(Request $request){
        ...
        $appointmentTempQuery = new Appointment;
        $QuotationTempQuery = new Quotation;
        $total_franchise = 0;
        if($userRole == 'Sub Contractor'){
            $appointment = $appointmentTempQuery->where('franchise_id', $franchiseID['id'])->get();
            $appointmentCount = $appointment->count();
            $total_franchise = $appointmentCount;
            $franchise = [];

            $quotations = $QuotationTempQuery->where('franchise_id', $franchiseID['id'])->get();
            $quotationCount = $quotations->count();
        }else{
