...
Route::post('/create-direct-customer', [CustomerController::class, 'createDirectCustomer'])->name('createDirectCustomer');
...
Route::post('/sub-contractor/active/{id}', [SubcontractorController::class, 'active'])->name('subcontractors.active');
...
Route::post('/convert-to-close-prospect', [CustomerController::class, 'closeAndProspect'])->name('closeAndProspect');
Route::post('/convert-to-close-dead', [CustomerController::class, 'closeAndDead'])->name('closeAndDead');
...
Route::middleware(['auth'])->prefix('bids')->group(function () {
    ...
    Route::get('/details/{id}/{type}', [QuotationController::class, 'getAppointmentDetails'])->name('quotations.details');
    Route::get('/download_quotes/{quotation_Id}', [QuotationController::class, 'downloadQuotationView']);
	Route::get('/edit/{quotation_Id}', [QuotationController::class, 'editQuotation']);
	 Route::put('/quotations/{quotation}',        [QuotationController::class,'update'])->name('quotation.update');
});

//jobs
Route::middleware(['auth'])->prefix('jobs')->group(function () {
    Route::get('/', [JobController::class, 'index'])->name('jobs.list');
    Route::get('data', [JobController::class, 'getQuotationsData']);
	Route::get('getSubContract/{apnt_id}', [AppointmentController::class, 'getSubContractList']);
	Route::post('/hold/work', [JobController::class, 'holdWork'])->name('hold.work');
	Route::post('/re-start/work', [JobController::class, 'restartWork'])->name('restart.work');
...
Route::middleware(['auth'])->prefix('contract')->group(function () {
    ...
    Route::get('create/{appointment_id}/{qoutation_id}', [ContractController::class, 'create'])->name('contract.create');
    Route::post('/store', [ContractController::class, 'store'])->name('contract.store');
    Route::post('contract/send/{appointment_id}', [ContractController::class, 'sendContractToCustomer'])->name('contract.send.email');
	Route::post('/preview', [ContractController::class, 'preview'])->name('contract.preview');
	Route::post('/confirm', [ContractController::class, 'confirm'])->name('contract.confirm');
...
// Customer Dashboard
Route::middleware(['auth'])->group(function () {
    Route::get('my-bid', [CustomerController::class, 'myBid'])->name('myBid.list');
    Route::get('my-bid-data', [CustomerController::class, 'getQuotationsData']);

    Route::get('my-contract', [ContractController::class, 'myContract'])->name('myContract.list');
    Route::get('/contract/view/{id}', [ContractController::class, 'viewContract'])->name('contract.view');
    Route::get('/contract/{id}/download', [ContractController::class, 'downloadPdf'])->name('contract.download');
});
...
Route::get('/approve/{token}', [ContractController::class, 'approve'])->name('contract.approve');
	Route::get('/comment/{token}', [ContractController::class, 'commentForm'])->name('contract.comment');
	Route::post('/comment/{token}', [ContractController::class, 'submitComment'])->name('contract.comment.submit');